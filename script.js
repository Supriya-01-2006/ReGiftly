...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if(donateForm){
    donateForm.addEventListener("submit", e=>{
      e.preventDefault();
      const f=e.target, name=f.name.value.trim(), email=f.email.value.trim(), details=f.details.value.trim();
      if(!name){showToast("Enter your name","error");return;}
      if(!isValidEmail(email)){showToast("Invalid email","error");return;}
      saveSubmission("donations",{name,email,details});
      f.reset(); showToast("Donation submitted!","success");
    });
  }
